...
exports.findTest = function(req, res) {
    console.log('findByTest');
    db.collection('markers', function(err, collection) {
        collection.find({'geometry': {'$near': 
              {'$geometry': 
                {'type': "Point", 
                 'coordinates': [-73.852801322937, 40.746899324307]}}, 
              '$maxDistance': 50000}}).toArray(function(err, items) { // $maxDistance is in meters. Pass through url
            console.log(items);
            // Format as Geojson
            var result = {"type": "FeatureCollection", "features": items};
            res.jsonp(result);
        });
    });
};
...
var populateDB = function() {
    ...
    console.log("Populating markers database...");
    var markers = [
        {"id": 1, "geometry": { "type": "Point", "coordinates": [-73.921508789062, 40.558678010243]}, "properties": {"title": "The Rockaways"}, "type": "Feature"},
        {"id": 2, "geometry": { "type": "Point", "coordinates": [-73.962364196777, 40.733990538532]}, "properties": {"title": "CSOs"}, "type": "Feature"},
        {"id": 3, "geometry": { "type": "Point", "coordinates": [-73.852801322937, 40.746899324307]}, "properties": {"title": "New York Hall of Science"}, "type": "Feature"},
    ];
    // var markers = [
    ...
    db.collection('markers', function(err, collection) {
        ...
        // not sure why two [[]]
        collection.createIndex([['geometry', '2dsphere']], function(err, indexName) {
            console.log("created index: " + indexName);  
        });
    });
    console.log("Finished populating markers database...");
